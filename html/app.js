...
Traphouses.Functions.ClosePinpad = function() {
    ...
    $.post(`https://${GetParentResourceName()}/PinpadClose`);
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        ...
        case 13:
            var v = $("#PINbox").val();
            if (v == "") {
                $.post(`https://${GetParentResourceName()}/ErrorMessage`, JSON.stringify({
                ...
                $("#PINbox").val("");
                $.post(`https://${GetParentResourceName()}/EnterPincode`, JSON.stringify({
                    pin: data.pin
                }))
                Traphouses.Functions.ClosePinpad();
            };
